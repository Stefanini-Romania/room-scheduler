...
{
    public class EditEventValidator : AbstractValidator<EditViewModel>
    {

        IRSManager rsManager;

        public EditEventValidator(IRSManager _rsManager)
        {
        ...
        private bool CanBook(EditViewModel ev, DateTime? d)
        {
        ...
        private bool CanCancel(EditViewModel ev, int attendee)
        {
            return rsManager.CanCancel((DateTime)ev.StartDate, (DateTime)ev.EndDate, ev.RoomId, attendee);
        }

        private bool TimeSpanOfEvent(EditViewModel ev, DateTime? d)
        {
            double eventTimeSpan = rsManager.GetTimeSpan((DateTime)ev.StartDate, (DateTime)ev.EndDate);
        ...
        private bool GoodTime(EditViewModel ev, DateTime? d)
        {
            if (d.HasValue)
            {
        ...
        private bool AvailabilityTimeS(EditViewModel ev, DateTime? d)
        {
            if (d.HasValue)
            {
        ...
        private bool AvailabilityTimeE(EditViewModel ev, DateTime? d)
        {
            if (d.HasValue)
            {
        ...
        private bool IsAvailable(EditViewModel ev, DateTime? d)
        {
            return rsManager.CheckAvailability((DateTime)ev.StartDate, (DateTime)ev.EndDate, ev.RoomId);
        }

        private bool IsNotPenalized(EditViewModel ev, DateTime? date)
        {
